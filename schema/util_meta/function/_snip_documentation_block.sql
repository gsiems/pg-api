CREATE OR REPLACE FUNCTION util_meta._snip_documentation_block (
...
AS $$
/* *
Function _snip_documentation_block generates the documentation block for an object
...
BEGIN
    ...
    l_return := concat_ws (
        ...
        concat_ws (
            ...
            a_object_purpose ),
        '',
        array_to_string ( l_doc_lines, util_meta._new_line () ),
        '' ) ;
    ...
    IF array_length ( a_notes, 1 ) > 0 THEN
        l_return := concat_ws (
            ...
            'NOTES',
            '',
            ' * ' || array_to_string ( a_notes, util_meta._new_line ( 2 ) || ' * ' ),
            '' ) ;
    ...
    IF array_length ( a_assertions, 1 ) > 0 THEN
        l_return := concat_ws (
            ...
            'ASSERTIONS',
            '',
            ' * ' || array_to_string ( a_assertions, util_meta._new_line ( 2 ) || ' * ' ),
            '' ) ;

    END IF ;

    l_return := concat_ws ( util_meta._new_line (), l_return, '*' || '/' ) ;
