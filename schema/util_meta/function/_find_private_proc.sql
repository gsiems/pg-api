CREATE OR REPLACE FUNCTION util_meta._find_private_proc (
...
AS $$
/* *
Function _find_private_proc searches for an existing "private" procedure for an API procedure.
...
BEGIN
    ...
    --l_base_schema_name := util_meta._base_name ( a_proc_schema ) ;
    FOR r IN (
        WITH n AS (
            SELECT schema_name,
                    a_proc_name AS proc_name,
                    l_full_object_name AS full_proc_name,
                    util_meta._base_name ( schema_name ) AS base_schema_name,
                    util_meta._base_order ( schema_name, util_meta._base_name ( schema_name ) ) AS base_order
        ...
        FOR r2 IN (
            SELECT obj.schema_name,
                    ...
                    --r.full_proc_name,
                    --r.schema_order,
                    --util_meta._base_order ( obj.object_name, r.proc_name ) AS base_object_order
                ...
                ORDER BY r.schema_order,
                    util_meta._base_order ( obj.object_name, r.proc_name ) ) LOOP
