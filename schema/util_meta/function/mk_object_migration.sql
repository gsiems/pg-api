...
BEGIN
   ...
   -- For non-tables the intent is to simply drop the old object and re-create it
    ...
    l_result := concat_ws ( l_new_line,
        l_result,
        '',
        concat_ws ( ' ', '-- Re-create objects that depend on', l_full_object_name, 'oid:', l_object_oid::text ) ) ;
    ...
    IF l_object_type = 'table' THEN
        ...
        l_result := concat_ws ( l_new_line,
            ...
            'RAISE NOTICE ''Remember to "DROP TABLE bak_' || l_full_object_name || ' ;" once the migration is verified'''
            ) ;

    END IF ;

    RETURN concat_ws ( l_new_line,
        l_result,
        '' ) ;
