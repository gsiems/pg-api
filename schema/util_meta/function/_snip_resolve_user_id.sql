CREATE OR REPLACE FUNCTION util_meta._snip_resolve_user_id (
...
AS $$
/* *
Function _snip_resolve_user_id generates the pl/pg-sql code snippet for performing user ID lookups
...
| a_user_id_param                | in     | text       | The parameter to check the user ID for (default a_act_user) |
...
BEGIN
    ...
    l_user_id_param := coalesce ( a_user_id_param, 'a_act_user' ) ;
    ...
    FOR r IN (
        ...
        l_return := util_meta._indent ( l_indents )
            || l_user_id_var
            || ' := '
            || r.full_object_name
            || ' ( a_username => '
        ...
        IF coalesce ( a_check_result, true ) THEN
            ...
            IF util_meta._is_valid_object ( 'util_log', 'log_exception', 'procedure' ) THEN
                l_log_err_line := util_meta._indent ( l_indents + 1 ) || 'call util_log.log_exception ( a_err ) ;' ;
            END IF ;

            l_return := concat_ws (
                util_meta._new_line (),
                l_return,
                util_meta._indent ( l_indents ) || 'IF ' || l_user_id_var || ' IS NULL THEN',
                util_meta._indent ( l_indents + 1 ) || 'a_err := ''No, or invalid, user specified'' ;',
                l_log_err_line,
                util_meta._indent ( l_indents + 1 ) || 'RETURN ;',
                util_meta._indent ( l_indents ) || 'END IF ;' ) ;
