CREATE OR REPLACE FUNCTION util_meta._snip_json_agg_build_object (
...
AS $$
/* *
Function _snip_json_agg_build_object generates a json_agg wrapped json_build_object function call query for a table or view
...
BEGIN
    ...
    IF util_meta._is_valid_object ( a_object_schema, a_object_name, 'view' ) THEN
        l_object_type := 'view' ;
    END IF ;

    IF l_object_type IS NULL THEN
        IF util_meta._is_valid_object ( a_object_schema, a_object_name, 'table' ) THEN
            l_object_type := 'table' ;
        END IF ;
    END IF ;
    ...
    l_select_cols := array_append (
        ...
        'json_agg ( json_build_object ('
            || util_meta._new_line ()
            || util_meta._indent ( l_indents + 5 )
            || array_to_string ( l_jbo_columns, ',' || util_meta._new_line () || util_meta._indent ( l_indents + 5 ) )
            || ' ) ) AS '
            || l_jbo_alias ) ;
    ...
    l_result := concat_ws (
        ...
        util_meta._indent ( l_indents + 1 )
            ...
            || array_to_string (
                l_group_cols,
                ',' || util_meta._new_line () || util_meta._indent ( l_indents + 3 ) ) ) ;
