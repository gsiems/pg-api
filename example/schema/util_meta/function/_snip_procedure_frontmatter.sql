CREATE OR REPLACE FUNCTION util_meta._snip_procedure_frontmatter (
...
AS $$
/* *
Function _snip_procedure_frontmatter generates the pl/pg-sql code snippet for the start of a procedure
...
BEGIN
    ...
    IF array_length ( a_variables.names, 1 ) > 0 THEN
        l_return := concat_ws (
            util_meta._new_line (),
            l_return,
            util_meta._snip_declare_variables ( a_variables => a_variables ),
            '' ) ;

    END IF ;

    l_return := concat_ws ( util_meta._new_line (), l_return, 'BEGIN' ) ;
