CREATE OR REPLACE FUNCTION util_meta._snip_function_frontmatter (
...
AS $$
/* *
Function _snip_function_frontmatter generates the pl/pg-sql code snippet for the start of a function
...
BEGIN
    ...
    IF l_param_count > 0 THEN
        ...
        l_return := concat_ws (
            ...
            array_to_string ( l_params, ', ' || util_meta._new_line () ) || ' )' ) ;
    ...
    IF coalesce ( a_returns_set, false ) THEN
        l_return := concat_ws ( util_meta._new_line (), l_return, 'RETURNS SETOF ' || a_return_type ) ;
    ELSE
        l_return := concat_ws ( util_meta._new_line (), l_return, 'RETURNS ' || a_return_type ) ;
    END IF ;
    ...
    IF l_is_stable THEN
        l_return := concat_ws ( util_meta._new_line (), l_return, 'STABLE' ) ;
    END IF ;
