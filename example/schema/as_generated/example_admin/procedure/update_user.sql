CREATE OR REPLACE PROCEDURE example_admin.update_user (
...
BEGIN
    ...
    IF NOT coalesce ( l_has_permission, false ) THEN
        a_err := 'No, or insufficient, privileges or the user does not exist' ;
        call util_log.log_exception ( a_err ) ;
        RETURN ;
    END IF ;

    call priv_example_admin.priv_update_user (
        a_id => a_id,
